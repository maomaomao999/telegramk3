...
def generate_xcodeproj(build_environment: BuildEnvironment, disable_extensions, disable_provisioning_profiles, include_release, generate_dsym, bazel_app_arguments, target_name):
    ...
    if developer_dir:
        project_bazel_arguments += [
            '--action_env=DEVELOPER_DIR={}'.format(developer_dir),
        ]
    ...
    if disable_provisioning_profiles:
        xcodeproj_extra_flags_path = os.path.join(
            xcodeproj_path,
            'rules_xcodeproj',
            'bazel',
            'xcodeproj_extra_flags.bazelrc',
        )
        with open(xcodeproj_extra_flags_path, 'w') as file:
            file.write('build --//{}:disableProvisioningProfiles\n'.format(app_target))
